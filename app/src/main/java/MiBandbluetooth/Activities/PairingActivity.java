...
//            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        ...
        DiscoveryActivity.discoveryActivity.finish();
        finish();
    }
    ...
    protected void performBluetoothPair(MiBandDevice deviceCandidate) {
        ...
        if (bondState == BluetoothDevice.BOND_BONDED) {
            AndroidUtils.toast(this, "Already connected to:  " + device.getName() + " (" + device.getAddress() + ")", Toast.LENGTH_SHORT);
            performApplicationLevelPair();
            return;
        }
        ...
        if (bondState == BluetoothDevice.BOND_BONDING) {
            AndroidUtils.toast(this, "Connecting to: " + device.getName() + " (" + bondingAddress + ")", Toast.LENGTH_LONG);
            return;
        }

        AndroidUtils.toast(this, "Connection successful to: " + device.getName() + " (" + bondingAddress + ")", Toast.LENGTH_LONG);
        if (!device.createBond()) {
            AndroidUtils.toast(this, "Connection failed to: " + device.getName() + " (" + bondingAddress + ")", Toast.LENGTH_LONG);
        }
    }
